...
- `IDENTITY.md` — the persona: name, creature emoji, avatar path, behavioral directives
- `SOUL.md` — domain expertise, personality boundaries, how the agent relates to the others
...
After fixing the binding schema and restarting the gateway, all five agents responded to @mentions in their respective channels. Success — or so I thought.
...
`ChannelMessage.Read.Group` tells Teams to deliver *all* channel messages to the bot, not just @mentions. But there's a catch that's easy to miss:
